...
import { FieldArray, reduxForm, reset, formValueSelector, InjectedFormProps } from 'redux-form';

import RenderFilter from 'src/scenes/alerts/filter/components/renderFilter';
import validate from 'src/scenes/alerts/filter/alertsFilter/alertsValidate';
...
class AlertsFilter extends React.Component<InjectedFormProps< any, Props> & Props, State> {

  render() {
    let { allProps, fieldTypeSelected, resetForm, handleSubmit } = this.props;

    // note: only one props can pass to fieldArray so added these props to allProps object.
...
const AlertsFilterConnected = connect<any>(
  mapStateToProps,
  mapDispatchToProps
)(AlertsFilter);
